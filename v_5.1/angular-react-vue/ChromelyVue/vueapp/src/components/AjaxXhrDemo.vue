...
<script>
    ...
    export default {
        setup() {
            ...
            const httpGetResultMap = ref([]);
            const httpPostResult = ref('Post request not ran or no result recieved.');

            const httpGetResult = computed(() => {
                return httpGetResultMap.value;
            })

            const httpGetRun = () => {
                axios.get('http://chromely.com/democontroller/movies/get')
                    .then(response => {
                        httpGetResultMap.value = parseArrayResult(response.data);
                    })
                    .catch(error => {
                        console.log(error);
                    });
